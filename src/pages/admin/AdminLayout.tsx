    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      <main className="flex-1 min-w-0">
        <div className="h-14 border-b bg-white px-4 flex items-center justify-between">
          ...
          <button onClick={handleLogout} className="rounded-md bg-red-600 px-3 py-1 text-sm text-white hover:bg-red-700">
            Logout
          </button>
        </div>
        <div className="p-6">
...
function AdminItem({ to, end, children }: { to: string; end?: boolean; children: React.ReactNode }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        ["block rounded-lg px-3 py-2 text-sm", isActive ? "bg-black text-white" : "text-gray-700 hover:bg-gray-100"].join(" ")
      }
    >
      {children}
    </NavLink>
